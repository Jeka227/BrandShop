...
class ControllerStartupSeoUrl extends Controller {
		...
		//seopro prepare route end
			...
			foreach ($parts as $part) {
                ...
                if($part && !$query->num_rows) {
                    if($this->model_extension_module_filter_vier->parseUrlSeoFV( $part )) {
                        continue;
                    }
                }
        		/*end FilterVier*/
	...
	public function rewrite($link) {
		...
		if ($data) {
                ...
                /*start FilterVier*/
                $this->load->model('extension/module/filter_vier');
                if($url_temp = $this->model_extension_module_filter_vier->getUrlSeoFV($url, $data)) {
                    if(isset($url_temp['seo_url'])) {
                        $url = $url_temp['seo_url'];
                        $data = $url_temp['new_data'];
                    }
                    if(isset($url_temp['arr_val_i'])) {
                        $data = array_merge($data, $url_temp['arr_val_i']);
                    }
                }
				if( $data ){$query='?'.urldecode(http_build_query($data, '', '&'));}
                /*next FilterVier*/
                /*
